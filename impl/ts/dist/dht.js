...
export function parseAgentRecordPayload(value) {
    const entries = expectMap(value);
    return {
...
export function parseCommunityRecord(value) {
    const [payload, signature] = splitSignedMap(value, 7);
    return { payload: parseCommunityRecordPayload(payload), signature };
}
export function buildAgentRecord(payload, secretKey) {
    return buildSignedRecord(agentRecordPayloadToCbor(payload), 5, secretKey);
}
export function buildServiceRecord(payload, secretKey) {
    return buildSignedRecord(serviceRecordPayloadToCbor(payload), 6, secretKey);
}
export function buildCommunityRecord(payload, secretKey) {
    return buildSignedRecord(communityRecordPayloadToCbor(payload), 7, secretKey);
}
export function verifyAgentRecord(data, publicKey) {
    return verifySignedRecord(data, 5, publicKey, parseAgentRecordPayload);
}
export function verifyServiceRecord(data, publicKey) {
    return verifySignedRecord(data, 6, publicKey, parseServiceRecordPayload);
}
export function verifyCommunityRecord(data, publicKey) {
    return verifySignedRecord(data, 7, publicKey, parseCommunityRecordPayload);
}
function agentRecordPayloadToCbor(payload) {
    return {
        entries: [
            [0, payload.agentId],
...
function serviceRecordPayloadToCbor(payload) {
    const entries = [
...
function communityRecordPayloadToCbor(payload) {
    const entries = [
...
function contactToCbor(contact) {
    return {
        entries: [
...
function buildSignedRecord(payload, sigKey, secretKey) {
...
function verifySignedRecord(data, sigKey, publicKey, parser) {
...
function expectBytesArrayLen(value, len) {
    if (!Array.isArray(value))
        throw new Error("expected bytes array");
    return value.map((item) => expectBytesLen(item, len));
}
function expectBytesLen(value, len) {
