...
export function decodeCanonical(data: Uint8Array): CborValue {
...
function decodeValue(data: Uint8Array, pos: number): [CborValue, number] {
  if (pos >= data.length) {
  ...
  if (major === 7) {
    if (addl === 20) return [false, pos];
    if (addl === 21) return [true, pos];
    if (addl === 22) return [null, pos];
...
function readLen(data: Uint8Array, pos: number, addl: number): [bigint, number] {
  if (addl <= 23) return [BigInt(addl), pos];
  if (addl === 24) {
    if (pos + 1 > data.length) throw new CborError("unexpected end of input");
    const val = BigInt(data[pos]);
    if (val < 24n) throw new CborError("non-canonical length");
    return [val, pos + 1];
  }
  if (addl === 25) {
    if (pos + 2 > data.length) throw new CborError("unexpected end of input");
    const val = BigInt((data[pos] << 8) | data[pos + 1]);
    if (val < 256n) throw new CborError("non-canonical length");
    return [val, pos + 2];
  }
  if (addl === 26) {
    if (pos + 4 > data.length) throw new CborError("unexpected end of input");
    const val = BigInt(
      (data[pos] * 0x1000000) +
        (data[pos + 1] << 16) +
        (data[pos + 2] << 8) +
    ...
    if (val < 65536n) throw new CborError("non-canonical length");
    return [val, pos + 4];
  }
  if (addl === 27) {
    if (pos + 8 > data.length) throw new CborError("unexpected end of input");
    const val =
      (BigInt(data[pos]) << 56n) |
      ...
      BigInt(data[pos + 7]);
    if (val < 4294967296n) throw new CborError("non-canonical length");
    return [val, pos + 8];
  }
  if (addl === 31) throw new CborError("indefinite length not allowed");
...
function encodeValue(value: CborValue, out: number[]): void {
  ...
  if (typeof value === "number") {
    if (!Number.isInteger(value)) throw new CborError("invalid integer");
...
function encodeInt(value: bigint, out: number[]): void {
  if (value >= 0n) {
...
function encodeMajor(out: number[], major: number, value: bigint): void {
  if (value < 0n) throw new CborError("invalid length");
  if (value <= 23n) {
