...
export function parseAgentMailPayload(value: CborValue): AgentMailMessagePayload {
  const entries = expectMap(value);
  const payload: AgentMailMessagePayload = {
...
export function parseAgentMailMessage(value: CborValue): AgentMailMessage {
  const [payload, signature] = splitSignedMap(value, AGENTMAIL_SIG_KEY);
  return { payload: parseAgentMailPayload(payload), signature };
}

export function decodeAgentMailMessage(data: Uint8Array): AgentMailMessage {
  return parseAgentMailMessage(decodeCanonical(data));
}

export function buildAgentMailMessage(payload: AgentMailMessagePayload, secretKey: Uint8Array): Uint8Array {
  validatePayload(payload);
...
export function verifyAgentMailMessage(data: Uint8Array, publicKey: Uint8Array): AgentMailMessagePayload {
...
function agentmailPayloadToCbor(payload: AgentMailMessagePayload): CborValue {
  validatePayload(payload);
  ...
  if (payload.threadId) entries.push([4, payload.threadId]);
  if (payload.replyTo) entries.push([5, payload.replyTo]);
  if (payload.subject) entries.push([6, payload.subject]);
  entries.push([7, payload.markdown]);
  if (payload.attachments) entries.push([8, payload.attachments.map(attachmentToCbor)]);
  if (payload.intentHashes) entries.push([9, payload.intentHashes.map((h) => new Uint8Array(h))]);
  if (payload.receiptHashes) entries.push([10, payload.receiptHashes.map((h) => new Uint8Array(h))]);
  if (payload.metadata !== undefined) entries.push([11, payload.metadata]);
  entries.push([12, payload.ts]);
  if (payload.expires !== undefined) entries.push([13, payload.expires]);
  return { entries } satisfies CborMap;
}

function attachmentToCbor(attachment: AgentMailAttachment): CborValue {
  ...
  if (attachment.retrieval) entries.push([3, attachment.retrieval.slice()]);
  return { entries } satisfies CborMap;
}

function parseAttachments(value: CborValue): AgentMailAttachment[] {
  if (!Array.isArray(value)) throw new Error("expected attachment array");
  if (value.length === 0) throw new Error("attachments empty");
  return value.map(parseAttachment);
}

function parseAttachment(value: CborValue): AgentMailAttachment {
  const entries = expectMap(value);
...
function expectHashArray(value: CborValue): Uint8Array[] {
  if (!Array.isArray(value)) throw new Error("expected hash array");
  if (value.length === 0) throw new Error("hash array empty");
  return value.map((item) => expectBytesLen(item, 32));
}

function validatePayload(payload: AgentMailMessagePayload): void {
  if (payload.version !== AGENTMAIL_VERSION) throw new Error("unsupported agentmail version");
  ...
  if (payload.threadId) ensureNonempty(payload.threadId, "thread_id required");
  if (payload.replyTo) ensureNonempty(payload.replyTo, "reply_to required");
  if (payload.subject) ensureNonempty(payload.subject, "subject required");
  ...
  if (payload.attachments) {
    if (payload.attachments.length === 0) throw new Error("attachments empty");
    payload.attachments.forEach(validateAttachment);
  }
  if (payload.intentHashes) {
    if (payload.intentHashes.length === 0) throw new Error("intent hashes empty");
    payload.intentHashes.forEach((hash) => {
      if (hash.length !== 32) throw new Error("intent hash length invalid");
    });
  }
  if (payload.receiptHashes) {
    if (payload.receiptHashes.length === 0) throw new Error("receipt hashes empty");
    payload.receiptHashes.forEach((hash) => {
      if (hash.length !== 32) throw new Error("receipt hash length invalid");
    });
  }
}

function validateAttachment(attachment: AgentMailAttachment): void {
  if (attachment.contentHash.length !== 32) throw new Error("attachment hash length invalid");
...
function expectU8(value: CborValue): number {
  ...
  if (num > 255) throw new Error("expected u8");
  return num;
}
